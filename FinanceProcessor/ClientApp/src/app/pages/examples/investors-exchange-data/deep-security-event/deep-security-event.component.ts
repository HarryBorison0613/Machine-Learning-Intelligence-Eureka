...
@Component({
  selector: "investors-exchange-data-deep-security-event",
  templateUrl: "deep-security-event.component.html"
})
export class DEEPSecurityEventComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.getSecurityEvent(this.selectedSymbol);
  }

  getSecurityEvent(query) {
    this.loading = true;
    this.data = [];
    let symbol = query.split('|')[1];
    this.http.post<any>(
      this.baseUrl + `InvestorsExchangeData/deepSecurityEvent?symbols=${symbol}`, httpOptions)
    .subscribe(result => {
