...
@Component({
  selector: "investors-exchange-data-deep-short-sale-price-test-status",
  templateUrl: "deep-short-sale-price-test-status.component.html"
})
export class DEEPShortSalePriceTestStatusComponent implements OnInit {

  data: ShortSalePriceTestStatus[];
  ...
  ngOnInit() {
    ...
    this.getShortSalePriceTestStatus(this.selectedSymbol);
  }

  getShortSalePriceTestStatus(query) {
    this.loading = true;
    this.data = [];
    let symbol = query.split('|')[1];
    this.http.post<any>(
      this.baseUrl + `InvestorsExchangeData/deepShortSalePriceTestStatus?symbols=${symbol}`, httpOptions)
    .subscribe(result => {
...
interface ShortSalePriceTestStatus {
  isSSR: boolean;
  detail: string;
