...
namespace FinanceProcessor.IEXSharp.Service.Cloud.CoreData.StockFundamentals
{
	public class StockFundamentalsService : IStockFundamentalsService
	{
		...
		public async Task<IEXResponse<IEnumerable<FundamentalValuationsResponse>>> FundamentalValuationsAsync(string symbol, TimeSeriesPeriod period = TimeSeriesPeriod.Quarterly, TimeSeries timeSeries = null)
		{
			const string urlPattern = "time-series/FUNDAMENTAL_VALUATIONS/[symbol]/[period]";
			...
			return await executor.ExecuteAsync<IEnumerable<FundamentalValuationsResponse>>(urlPattern, pathNvc, qsb);
		}
