...
namespace FinanceProcessor.IEXSharp.Service.Cloud.CoreData.StockResearch
{
	public class StockResearchService : IStockResearchService
	{
		...
		public async Task<IEXResponse<RelevantStocksResponse>> RelevantStocksAsync(string symbol) =>
			await executor.SymbolExecuteAsync<RelevantStocksResponse>("stock/[symbol]/relevant", symbol);

		public async Task<IEXResponse<TechnicalIndicatorsResponse>> TechnicalIndicatorsAsync(string symbol, IndicatorName indicatorName, ChartRange range, bool lastIndicator = false, bool indicatorOnly = false)
		{
			...
			var pathNvc = new NameValueCollection { { "symbol", symbol }, { "indicator", indicatorName.GetDescriptionFromEnum() } };
